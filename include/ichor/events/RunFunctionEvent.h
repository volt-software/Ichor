...
namespace Ichor {
    struct RunFunctionEventAsync final : public Event {
        RunFunctionEventAsync(uint64_t _id, uint64_t _originatingService, uint64_t _priority, std::function<AsyncGenerator<IchorBehaviour>(DependencyManager&)> _fun) noexcept : Event(TYPE, NAME, _id, _originatingService, _priority), fun(std::move(_fun)) {}
        ~RunFunctionEventAsync() final = default;

        std::function<AsyncGenerator<IchorBehaviour>(DependencyManager&)> fun;
