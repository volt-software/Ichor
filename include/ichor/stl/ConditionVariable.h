...
namespace Ichor {
    ...
    struct ConditionVariable final {
        ...
        cv_status wait_until(std::unique_lock<RealtimeMutex>& lock, const std::chrono::time_point<ClockT, DurationT>& atime)
        {
            static_assert(std::chrono::is_clock_v<ClockT>);
            const typename ClockT::time_point _c_entry = ClockT::now();
