...
class printer
{
	...
	virtual unsigned int get_indent_level() const
	{
		if (indent_level < 0) return 0;
		return static_cast<unsigned int>(indent_level);
	}
			...
			!= option_style::opt_print_order::per_declaration)
		{
			...
			std::stable_sort(order_index.begin(), order_index.end(),
				[&](std::size_t i, std::size_t j) {
					const parser & pa = **(b + static_cast<decltype(order_index)::difference_type>(i));
					const parser & pb = **(b + static_cast<decltype(order_index)::difference_type>(j));
					return style.opt_print_order_less(
						pa.get_print_order_key(style),
						pb.get_print_order_key(style));
				});
			for (auto i : order_index) f(style, **(b + static_cast<decltype(order_index)::difference_type>(i)));
		}
		else
		{
			while (b != e) f(style, **(b++));
