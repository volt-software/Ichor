...
Ichor::AsyncAutoResetEvent::AsyncAutoResetEvent(bool initiallySet) noexcept
...
Ichor::AsyncAutoResetEventOperation
    ...
    if (local::get_set_count(oldState) > local::get_waiter_count(oldState)) {
        ...
        if (m_state.compare_exchange_strong(oldState, oldState - local::set_increment, std::memory_order_acquire, std::memory_order_relaxed)) {
            // Acquired the event, return an operation object that won't suspend.
...
Ichor::AsyncAutoResetEventOperation::AsyncAutoResetEventOperation() noexcept
        : m_event(nullptr)
{}

Ichor::AsyncAutoResetEventOperation::AsyncAutoResetEventOperation(
